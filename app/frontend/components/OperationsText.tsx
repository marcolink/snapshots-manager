...
import {numberToWords} from "~/frontend/utils/number-to-words";
import {PatchEntryWithUser} from "~/shared/types";
import {createMetadataChange} from "~/frontend/utils/patch-utils";

const pluralize = (word: string, count: number) => count === 1 ? word : `${word}s`

type OperationsTextProps = {
  entry: PatchEntryWithUser
  locales: string[]
}

export function OperationsText({entry}: OperationsTextProps) {
