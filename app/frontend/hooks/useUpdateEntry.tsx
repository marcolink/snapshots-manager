...
import {createFieldChange} from "~/frontend/utils/patch-utils";
    ...
    mutationFn: async (entry: PatchEntryWithUser) => {
      ...
      const shouldExecute = await ModalLauncher.open(({isShown, onClose}) => {
        return (
          ...
          >
            <Text>
              This will apply the <strong>{(entry.patch as Patch).length} change(s)</strong> from the selected snapshot
              on top of the current entry.
            </Text>
          </ModalConfirm>
        );
      })
